...
# Define helper function for file renames
# USAGE: perl_rename 'perl_expression' file1 file2 ...
# EXAMPLE: perl_rename 's/my\-app/$NAME/g' bin/my-app-cdk.ts
perl_rename() {
  local expr=$1
  shift
  for file in "$@"; do
    local new_name=$(perl -e "(\$name = '$file') =~ $expr; print \$name")
...
perl -i -pe "s/my\-app/$NAME/g" scripts/run-certbot-live.sh
...
perl_rename "s/my\-app/$NAME/g" lib/my-app-cdk-stack.ts
perl_rename "s/my\-app/$NAME/g" bin/my-app-cdk.ts
perl_rename "s/my\-app/$NAME/g" test/my-app-cdk.test.ts
cd ../create-certs-cdk
rm -f .gitignore
mv USE_THIS.gitignore .gitignore
cp .env/RENAME_TO.secrets.js .env/.secrets.js
...
rm -f .gitignore
mv USE_THIS.gitignore .gitignore
perl_rename "s/my\-app/$NAME/g" cdk/my-app-cdk
perl -i -pe 's/("version": ")([0-9.]+)(",)/${1}0.0.0$3/g' package.json
