...
const defaultPasswordForOne = process.env.SECRET_KEY_ONE
...
describe('app server', () => {
        ...
        describe('should set secure headers', () => {
          ...
          it(`should set header "content-security-policy" as expected`, () => {
            expect(res.headers['content-security-policy']).toEqual(
              "base-uri 'self';block-all-mixed-content;connect-src 'self';default-src 'none';font-src 'self' https: data:;form-action 'self';frame-ancestors 'none';frame-src 'none';img-src 'self' data:;media-src 'none';object-src 'none';sandbox allow-forms allow-scripts allow-same-origin allow-popups;script-src 'self' 'unsafe-eval';script-src-attr 'none';style-src 'self' https: 'unsafe-inline';upgrade-insecure-requests",
  ...
  describe(`WHEN POST ${unlockPath} with default secret for variation one`, () => {
    ...
    beforeAll(async () => {
      res = await request(app)
        ...
        .send(`{"${process.env.KEY_NAME}":"${defaultPasswordForOne}"}`)
    })
    ...
    describe(`WHEN subsequently GET / with a valid cookie for variation one`, () => {
